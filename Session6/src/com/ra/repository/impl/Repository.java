...
public class Repository<T,K> implements IRepository<T,K> {
    ...
    @Override
    public T add(T entity,Class<T> entityClass) {
        Connection conn = null;
        PreparedStatement pst = null;
        ResultSet rs = null;
        try {
            ...
            String sql = MessageFormat.format("INSERT INTO {0}({1}) VALUES ({2})", entityClass.getAnnotation(Table.class).name(), columns,values);
            int index = 1;
            pst = conn.prepareStatement(sql);
            for (Field f : fields) {
                f.setAccessible(true);
